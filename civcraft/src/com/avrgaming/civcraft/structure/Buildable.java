...
public abstract class Buildable extends SQLObject {
	...
	public Map<BlockCoord, StructureSign> structureSigns = new ConcurrentHashMap<BlockCoord, StructureSign>();
	public Map<BlockCoord, StructureChest> structureChests = new ConcurrentHashMap<BlockCoord, StructureChest>();
	public Map<BlockCoord, StructureTables> structureTables = new ConcurrentHashMap<BlockCoord, StructureTables>();
...
//		undo_tpl.buildUndoTemplate(undo_tpl, this.getCorner().getBlock());
	...
	public void unbindStructureComponents() {
		...
		for (StructureTables table : this.structureTables.values()) {
			CivGlobal.removeStructureTable(table);
		}
		
		for (StructureChest chest : this.structureChests.values()) {
			CivGlobal.removeStructureChest(chest);
		}
		
		for (StructureSign sign: this.structureSigns.values()) {
			CivGlobal.removeStructureSign(sign);
		}
	}
...
//		}
	...
	public void addStructureTable(StructureTables table) {
		this.structureTables.put(table.getCoord(), table);
	}
	...
	public StructureChest getAChestById(int id) {
		for (StructureChest chest : this.structureChests.values()) {
			if (chest.getChestId() == id) {
				return chest;
			}
	...
	public void flashStructureBlocks() {
		for (BlockCoord coord : structureBlocks.keySet()) {
			...
			if (c1 == c2) {
				World world = coord.getLocation().getWorld();
