...
/*	public void spawnSugarcane(BlockSnapshot bs, CivAsyncTask task) throws InterruptedException {
		...
		try {
			...
			if (nextBlock.getTypeId() == CivData.AIR && bs.getTypeId() == CivData.SUGARCANE_BLOCK) {
				addGrowBlock("world", nextBlock.getX(), nextBlock.getY(), nextBlock.getZ(), CivData.SUGARCANE_BLOCK, 0, true);
			}
		} catch (InvalidBlockLocation e) {
			e.printStackTrace();
		}
...
//			if (CivData.canSugarcaneGrow(bs)) {
//				spawnSugarcane(bs, task);
//			}
		}
	}
					...
					for (int y = 0; y < 256; y++) {					
						...
						if (CivData.canGrow(bs)) {
							this.cropLocationCache.add(new BlockCoord(snapshot.getWorldName(), 
								(snapshot.getX() << 4) + bs.getX(), (bs.getY()), (snapshot.getZ() << 4) + bs.getZ()));
						}					
					}
				}
			}
