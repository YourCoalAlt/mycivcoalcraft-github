...
public class UnitItemMaterial extends LoreMaterial {
	...
	@Override //XXX If a unit is destroyable, do it here.
	public void onPlayerDeath(EntityDeathEvent event, ItemStack stack) {
		Player p = (Player)event.getEntity();
		Random r = new Random();
		int randChance = r.nextInt(101);
		if (War.isWarTime()) {
			CivMessage.send(p, CivColor.YellowBold+"Your unit has been kept alive since it is War Time.");
		}
		
		ConfigUnit u = CivSettings.units.get(Unit.getPlayerUnit(p).id);
		int dmg = u.destroy_chance;
		if (dmg < randChance && !(dmg <= 0)) {
			Unit.removeUnit(p);
			CivMessage.send(p, CivColor.RoseBold+"Your "+u.name+" has been destroyed! Rolled: "+dmg+" < "+randChance);
		}
	}
