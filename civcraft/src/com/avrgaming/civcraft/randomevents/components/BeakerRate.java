...
public class BeakerRate extends RandomEventComponent {
	...
	public void process() {
		...
		if (rate > 1.0) {
			sendMessage("Our science rate has increased by "+df.format((rate - 1.0)*100)+"% due to an unforseen event!");
		} else {
			sendMessage("Our science rate has decreased by "+df.format((1.0 - rate)*100)+"% due to an unforseen event!");
		}
	}
