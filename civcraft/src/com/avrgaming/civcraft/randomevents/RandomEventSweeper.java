		...
		for (RandomEvent event : events) {
			...
			if (event.requirements.size() > 0) {
				...
				for (RandomEventComponent comp : event.requirements.values()) {
					if (!comp.onCheck()) allPass = false;
				}
			}
			
			if (allPass) {
				/* Process success and then cleanup event. */
				for (RandomEventComponent comp : event.success.values()) comp.process();
				event.cleanup();
				removed.add(event);
			} else {
				...
				if (now.getTime() > expireTime) {
					...
					for (RandomEventComponent comp : event.failure.values()) comp.process();
					event.cleanup();
					removed.add(event);
				}
			}			
