...
public class ConfigLumberMill {
	...
	public static void loadConfig(FileConfiguration cfg, ArrayList<ConfigLumberMill> configList) {
		configList.clear();
		List<Map<?, ?>> drops = cfg.getMapList("lumbermill_drops");
		for (Map<?, ?> item : drops) {
			ConfigLumberMill g = new ConfigLumberMill();
		...
		CivLog.info("Loaded "+configList.size()+" Lumber Mill Drops.");
	}
}
