...
public class ConfigTrommel {
	...
	public static void loadConfig(FileConfiguration cfg, ArrayList<ConfigTrommel> configList) {
		configList.clear();
		List<Map<?, ?>> drops = cfg.getMapList("trommel_drops");
		for (Map<?, ?> item : drops) {
			ConfigTrommel g = new ConfigTrommel();
		...
		CivLog.info("Loaded "+configList.size()+" Trommel Drops.");
	}
}
