...
public class CivPerms {
	...
	public static void validControl(Player p) throws CivException {
		if (!p.isOp() || !p.hasPermission(CONTROL) ||
				p.getUniqueId().toString() != "a8e4c9db-3f73-41ae-ba65-65620c65e8b3") {
	...
	public static void validAdminOP(Player p) throws CivException {
		if (!p.isOp() || !p.hasPermission(CONTROL) || !p.hasPermission(ADMIN_OP)) {
	...
	public static void validRanks(Player p) throws CivException {
		if (!p.isOp() || !p.hasPermission(CONTROL) || !p.hasPermission(ADMIN_OP) || !p.hasPermission(RANKS)) {
