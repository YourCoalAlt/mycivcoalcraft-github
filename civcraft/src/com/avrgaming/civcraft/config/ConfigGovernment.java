...
public class ConfigGovernment {
	...
	public static void loadConfig(FileConfiguration cfg, Map<String, ConfigGovernment> government_map) {
		...
		for (Map<?, ?> level : techs) {
			...
			gov.housing_rate = (Double)level.get("housing_rate");
			gov.upkeep_rate = (Double)level.get("upkeep_rate");
			gov.max_tax_rate = (Double)level.get("maximum_tax_rate");
			gov.trommel_process_rate = (Double)level.get("trommel_process_rate");
			gov.quarry_process_rate = (Double)level.get("quarry_process_rate");
