...
public class Civilization extends SQLObject {
	...
	private Map<String, ConfigTech> techsIP = new ConcurrentHashMap<String, ConfigTech>();
	...
	public Civilization(String name, String capitolName, Resident leader) throws InvalidNameException {
		this.setName(name);
		this.leaderName = leader.getUUIDString();
	...
	public static void init() throws SQLException {
		if (!SQL.hasTable(TABLE_NAME)) {
			String table_create = "CREATE TABLE " + SQL.tb_prefix + TABLE_NAME+" (" + 
					...
					"`techsProgress` mediumtext DEFAULT NULL," +
	...
	public void load(ResultSet rs) throws SQLException, InvalidNameException {
		...
		if (taxes > this.government.max_tax_rate) {
			taxes = this.government.max_tax_rate;
		}
		this.setIncomeTaxRate(taxes);
		
		this.loadResearchedTechs(rs.getString("researched"));
		this.loadTechsInProgress(rs.getString("techsProgress"));
	...
	public void saveNow() throws SQLException {
		...
		hashmap.put("techsProgress", this.saveTechsInProgress());
	...
	private void loadTechsInProgress(String techstring) {
		if (techstring == null || techstring.equals("")) {
			return;
		}
	...
	private Object saveTechsInProgress() {
		String out = "";
		for (ConfigTech tech : this.techsIP.values()) {
			out += tech.id+",";
		}
		return out;
	}
	...
	public boolean hasTechnologyInProgress(String require_tech) {
		if (require_tech != null) {
			String split[] = require_tech.split(":");
			for (String str : split) {
				if (!hasTechInProgress(str)) {
					return false;
				}
			}
		}
	...
	private boolean hasTechInProgress(String configId) {
		if (configId == null || configId.equals("")) {
			return true;
		}
		return techsIP.containsKey(configId);
	}
	...
	public void removeTechFromProgress(ConfigTech t) {
		removeTechFromProgress(t.id);
	}
	
	public void removeTechFromProgress(String configId) {
		techsIP.remove(configId);
		for (Town town : this.getTowns()) {
			town.onTechUpdate();
		}
	}
	...
	public void setGovernment(String gov_id) {
		...
		if (this.getSciencePercentage() > this.government.max_tax_rate) {
			this.setSciencePercentage(this.government.max_tax_rate);
		}
