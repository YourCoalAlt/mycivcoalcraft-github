			...
			if (event.getCurrentItem() != null) {
				if (event.getCurrentItem().hasItemMeta()) {
					if (event.getCurrentItem().getItemMeta().hasDisplayName()) {
						if (event.getCurrentItem().getItemMeta().getDisplayName().toString().toLowerCase().contains("backpack")) {
							...
							case UNKNOWN:
								CivMessage.sendError(event.getWhoClicked(), "Cannot move your Backpack inside your inventory.");
								event.setCancelled(true);
								event.setResult(Result.DENY);
								break;
							default:
								CivMessage.sendError(event.getWhoClicked(), "{default_error} Cannot move your Backpack inside your inventory.");
								event.setCancelled(true);
								event.setResult(Result.DENY);
								break;
							}
...
//						event.setCancelled(true);
			...
			if (craftMat != null) {
				...
				//TODO set random change of doing 0.5 dmg?
	...
	public void onItemPickup(PlayerPickupItemEvent event) {
		...
		if (ItemManager.getId(event.getItem().getItemStack()) == ItemManager.getId(Material.RAW_FISH)
				&& ItemManager.getData(event.getItem().getItemStack()) == 
					ItemManager.getData(ItemManager.getMaterialData(CivData.FISH_RAW, CivData.DATA_2))) {
			LoreCraftableMaterial craftMat = LoreCraftableMaterial.getCraftMaterial(event.getItem().getItemStack());
			if (craftMat == null) {
				// Found a vanilla slime ball.
