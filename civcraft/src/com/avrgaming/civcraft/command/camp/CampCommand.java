...
public class CampCommand extends CommandBase {
	...
	public void init() {
		...
		commands.put("refresh", "Refreshes the camp structure. WARNING! All items in inventories will be popped out.");
	}
	
	public void refresh_cmd() throws CivException {
		Resident resident = getResident();
		if (!resident.hasCamp()) {
			throw new CivException("You are not apart of a camp.");
		}
		
		Camp camp = resident.getCamp();
		if (camp.getOwner() != resident) {
			throw new CivException("You must be the owner of a camp.");
		}
		
		if (camp.isDestroyed()) {
			throw new CivException("Camp is destroyed?");
		}
		
		try {
			camp.repairFromTemplate();
//			camp.reprocessCommandSigns();
			CivMessage.sendSuccess(sender, "Camp has been refreshed!");
	...
	public void undo_cmd() throws CivException {
		...
		for (ItemStack stack : leftovers.values()) {
			...
			CivMessage.send(player, CivColor.Gray+"Your camp item was dropped on the ground because your inventory was full.");
		}
