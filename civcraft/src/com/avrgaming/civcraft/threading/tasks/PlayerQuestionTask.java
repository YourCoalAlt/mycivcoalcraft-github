...
//	RunnableWithArg finishedTask; /* Task to run when a response has been generated. */
	...
	@Override
	public void run() {	
		CivMessage.send(askedPlayer, CivColor.Gray+"Question from: "+CivColor.LightBlue+questionPlayer.getName());
		CivMessage.send(askedPlayer, CivColor.LightPurpleBold+question);
		CivMessage.send(askedPlayer, CivColor.Gray+"Respond by typing "+CivColor.LightBlue+"/accept"+CivColor.Gray+" or "+CivColor.LightBlue+"/deny");
		...
		CivMessage.send(askedPlayer, CivColor.Gray+"You failed to respond to the question from "+questionPlayer.getName()+" in time.");
		CivMessage.send(questionPlayer, CivColor.Gray+askedPlayer.getName()+" failed to answer the question in time.");
		cleanup();
	}
