...
public class CivQuestionTask implements Runnable {
	...
	public void askPlayer(Player player) {
		CivMessage.send(player, CivColor.Gray+"Request from: "+CivColor.LightBlue+questionCiv.getName());
		CivMessage.send(player, question);
		CivMessage.send(player, CivColor.Gray+"Respond by typing "+CivColor.LightBlue+"/civ dip respond yes"+CivColor.Gray+" or "+CivColor.LightBlue+"/civ dip respond no");
	}
	
	public void notifyExpired(Player player) {
		CivMessage.send(player, CivColor.Gray+"The offer from "+questionCiv.getName()+" expired.");
	}
	...
	public void run() {
		...
		CivMessage.sendCiv(questionCiv, CivColor.Gray+askedCiv.getName()+" gave no response to our offer.");
		cleanup();		
	}
