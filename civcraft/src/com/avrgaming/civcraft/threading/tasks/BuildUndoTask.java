...
public class BuildUndoTask implements Runnable {
		...
		public void build(Queue<SimpleBlock> syncBlockQueue, SimpleBlock sb) throws InterruptedException {
			builtBlockCount++;
			if (ParticleEffectTimer.externalParticleBlocks.get(cornerBlock.getLocation()) != null) {
				ParticleEffectTimer.externalParticleBlocks.remove(cornerBlock.getLocation());
			}
			
			if (builtBlockCount < savedBlockCount) {
		...
		public static void resumeUndoTasks() {
			...
			for (File f : folder.listFiles()) {
					try {
							...
							/* Resume undo task after 10 seconds. Give server some time to start. */
							TaskMaster.asyncTask(task, TimeTools.toTicks(10));
							br.close();
					} catch (IOException e) {
